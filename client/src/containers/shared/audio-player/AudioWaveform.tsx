...
import { rgbLinearShade, hexToRGB } from '../../../styles/utils';
...
const AudioWaveform = (props: AudioWaveformProps) => {
  ...
  const commitSeek = (progress) => {
    if (
      lastSeekCommit &&
      lastSeekCommit >= Date.now() - MAX_SEEK_FREQUENCY_IN_MS
    )
      return;
      ...
      0 +
        x *
          (SizeConfiguration(props.size).barWidth +
            SizeConfiguration(props.size).barSpacing),
      WAVEFORM_MAX_HEIGHT,
      SizeConfiguration(props.size).barWidth,
      -height,
    );
  ...
  useEffect(() => {
    setColorPalette({
      ...
      hover: rgbLinearShade(-0.2, hexToRGB(props.accentColor, 1)),
      played: rgbLinearShade(-0.4, hexToRGB(props.accentColor, 1)),
    });
  }, [props.accentColor]);
  ...
  useEffect(() => {
    if (props.size !== WaveformSize.Unknown) {
      setWaveformSamples(
        props.waveform
          .filter(
            (_, i) => i % SizeConfiguration(props.size).samplingModulo === 0,
          )
          .map((amplitude) => ({
            amplitude,
            loaded: false,
  ...
  useEffect(() => {
    ...
    ctx.clearRect(
      0,
      0,
      SizeConfiguration(props.size).canvasWidth,
      WAVEFORM_MAX_HEIGHT,
    );
    waveformSamples.forEach((sample, x) => {
      const normalizedAmplitude = Math.ceil(
        (sample.amplitude / 128) * WAVEFORM_MAX_HEIGHT,
      );
      const fill = (height) => {
        // base case (sample animation complete)
        if (height > normalizedAmplitude) {
          setWaveformSamples((prevWaveformSamples) =>
            prevWaveformSamples.map((prevSample, i) => ({
              ...prevSample,
              animated: prevSample.animated || i === x,
            })),
          );
          return;
        }
  ...
  const handleMouseMove = (e) => {
    ...
    if (seeking) {
      updateWaveformSamples((sample, i) => ({
        ...sample,
        played: i <= samples,
      }));
      commitSeek(progress);
